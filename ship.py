...
class Ship(base.BaseClass):
	...
	vector=self["location"].displace(self["speed"]*20, self["direction"])
	...
	if self["debug"]:
	    ...
	    txt="%s @ %s -> %0.2f speed=%0.2f %s" % (self, here, self["direction"], self["speed"], self["mode"])
	    text=font.render(txt, 1, (255,255,255))
	    textpos=text.get_rect(centerx=screen.get_width()/2)
	    screen.blit(text, textpos)

	    txt="%s @ %s angle=%0.2f distance=%d %s" % (self.target_ship, self.target_ship["location"].icoords(), self["location"].angle(self.target_ship["location"]), self.distance(self.target_ship), self.target_ship["mode"])
	    text=font.render(txt, 1, (255,255,255))
	    textpos=text.get_rect(centerx=screen.get_width()/2, top=20)
	    screen.blit(text, textpos)
	    pygame.draw.aaline(screen, (255,255,255), here, self.target_ship["location"].icoords())
	...
	if not enemy:
		...
		if self.distance(enemy)<c["range"]:
			...
			here=self["location"].icoords()
			pygame.draw.aaline(self["screen"], (255,255,255), here, self.target_ship["location"].icoords())
    ...
    def inflict_damage(self, dmg):
    	shielders=[c for c in self.components if "shield" in c and c["shield"]>0]
	if shielders:
	    s=choice(shielders)
	else:
	    s=None
    ...
    ############################################################################
    def move(self):
    	self["location"]=self["location"].displace(self["speed"], self["direction"])
